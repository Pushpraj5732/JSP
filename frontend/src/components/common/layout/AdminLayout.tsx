                    ...
                    {sidebarItems.map((item) => (
                        <Link
                            ...
                            className={`flex items-center gap-2 px-4 h-full border-b-2 transition-colors ${location.pathname === item.path
                                    ? 'border-dark text-dark font-heading font-bold'
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.05)] h-16 flex items-center justify-around z-50 pb-safe">
                {sidebarItems.map((item) => (
                    <Link
                        key={item.path}
                        to={item.path}
                        className={`flex flex-col items-center justify-center w-full h-full gap-1 text-[10px] font-heading font-semibold transition-colors ${location.pathname === item.path ? 'text-dark bg-slate-50' : 'text-txt-muted'
                            }`}
                    >
                        <item.icon className={`w-5 h-5 ${location.pathname === item.path ? 'fill-navy/10' : ''}`} />
